...
def upgrade() -> None:
    ...
    op.create_table('user_profile_genre',
    sa.Column('id', sa.Uuid(), nullable=False, comment='Идентификатор связи пользователя и жанра'),
    sa.Column('created', sa.DateTime(), nullable=False, comment='Время создания записи'),
    sa.Column('modified', sa.DateTime(), nullable=False, comment='Время изменения записи'),
    ...
    sa.ForeignKeyConstraint(['genre_id'], ['genre.id'], name=op.f('fk_user_profile_genre_genre_id_genre'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['user_id'], ['user_profile.id'], name=op.f('fk_user_profile_genre_user_id_user_profile'), ondelete='CASCADE'),
